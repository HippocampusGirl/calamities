...
class SingleChoiceInputView(CallableView):
    def __init__(self, options, label=None,
                 cur_index=None, isVertical=False,
                 addBrackets=True,
                 showSelectionAfterExit=True,
                 renderfun=None, colorfun=None,
                 **kwargs):
    ...
    def set_options(self, newoptions):
        ...
        if self.addBrackets:
            for i in range(len(newoptions)):
                if isinstance(newoptions[i], TextElement):
                    newoptions[i].value = f"[{newoptions[i].value}]"
                elif isinstance(newoptions[i], TextElementCollection):
                    newoptions[i].textElements[0].value = \
                        f"[{newoptions[i].textElements[0].value}"
                    newoptions[i].textElements[-1].value = \
                        f"{newoptions[i].textElements[-1].value}]"
                else:
                    raise NotImplementedError
        if self.options is not None:
            self.clearBeforeDrawSize = len(self.options)
        self.options = newoptions
        if self.cur_index is not None and \
                self.cur_index > len(newoptions):
            self.cur_index = 0
    ...
    def _handleKey(self, c):
        if c == Key.Break:
            ...
            self.cur_index = \
                min(len(self.options) - 1, self.cur_index + 1)
            self.update()
    ...
    def _drawAt_horizontal(self, y):
        ...
        for i, option in enumerate(self.options):
            ...
            nchr = option.drawAt(y, x, self.layout, color,
    ...
    def _draw_text(self, y, text, color):
        ...
        self.layout.window.addstr(
            y, nchr, nothing, self.layout.color.default)
        return nchr
    ...
    def _draw_option(self, i, y):
        ...
        nchr = option.drawAt(y, 0, self.layout, color,
    ...
    def _drawAt_vertical(self, y):
        ...
        if self.cur_index is not None:
            ...
            while prev_offset != self.offset:
                ...
                (haveMoreAtFront,
                 haveMoreAtEnd,
                 correctedMaxSize) = _calc_layout()

        if haveMoreAtFront:
            entry = TextElement(f"-- {self.offset} more --",
                                self.layout.color.default)
...
class MultipleChoiceInputView(SingleChoiceInputView):
    def __init__(self, options, checked=[], isVertical=False, **kwargs):
        super(MultipleChoiceInputView, self).__init__(
            ...
            **kwargs)
        self.checked = {
            str(k): (str(k) in checked) for k in self.options
        }
    ...
    def _before_call(self):
        ...
        self._setStatusBar("  ".join([
            u"[↵] Ok",
            u"[space] Toggle checked/unchecked",
            f"[{arrows}] Change selection",
            u"[ctrl-c] Cancel"
        ]))
...
class MultiSingleChoiceInputView(SingleChoiceInputView):
        ...
        if addBrackets:
            for i in range(len(values)):
                if isinstance(values[i], TextElement):
                    ...
                    values[i].textElements[0].value = \
                        f"[{values[i].textElements[0].value}"
                    values[i].textElements[-1].value = \
                        f"{values[i].textElements[-1].value}]"
                else:
                    raise NotImplementedError
        self.values = values
    ...
    def _handleKey(self, c):
        if c == Key.Left:
            if self.cur_index is not None and \
                    self.selectedIndices is not None:
                self.selectedIndices[self.cur_index] = \
                    max(0, self.selectedIndices[self.cur_index] - 1)
            self.update()
        elif c == Key.Right:
            if self.cur_index is not None and \
                    self.selectedIndices is not None:
                self.selectedIndices[self.cur_index] = \
                    min(len(self.values) - 1,
        ...
        for j, value in enumerate(self.values):
            ...
            if self.selectedIndices is not None:
                if j == self.selectedIndices[i]:
                    if self.cur_index is not None and \
                            i == self.cur_index and self.isActive:
                        color = self.emphasisColor
                    else:
                        color = self.highlightColor
...
class MultiMultipleChoiceInputView(MultiSingleChoiceInputView):
    def __init__(self, options, values, checked=[], **kwargs):
        ...
        self.checked = [
            {
                str(k): (str(k) in checked) for k in self.values
            }
    ...
    def _handleKey(self, c):
        if c == Key.Left:
            ...
            if self.cur_index is not None and \
                ...
                self.checked[self.cur_index][valueStr] = \
                    not self.checked[self.cur_index][valueStr]
                self.update()
        else:
            super(MultiSingleChoiceInputView, self)._handleKey(c)
    ...
    def _draw_option(self, i, y):
        ...
        for j, value in enumerate(self.values):
            ...
            color = self.color
            if self.cur_col is not None and j == self.cur_col and \
